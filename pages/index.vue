<template>
  <main>
    <div
      ref="first"
      class="min-h-screen pt-24 flex items-start relative overflow-hidden bg-image-bib bg-center bg-cover text-blue-500"
    >
      <div
      ...
      <div class="container pt-8 md:pt-24 z-10">
        <div class="md:w-3/4">
          <img
            class="w-24 md:w-40 h-24 md:h-40 rounded-full float-right border-green-500 border-4 md:border-8 ml-2"
            src="/img/profile.jpg"
            :alt="$t('home.altProfile')"
          />
          <h1
            class="text-3xl md:text-6xl md:leading-none md:w-2/3 font-black mb-8"
        ...
        ></p>
        <button
          class="bg-green-500 p-2 md:p-4 md:text-lg text-white font-medium"
      ...
      <div
        class="absolute right-4 bottom-4 bg-green-300 bg-opacity-90 p-2 rounded font-semibold text-green-900 text-sm"
      >
        {{ $t('home.captionBib') }}
      </div>
    </div>
    <div
      id="about"
      ref="about"
      class="bg-green-500 w-full text-white py-4 md:py-8 space-y-8"
    >
      <h2 class="text-2xl md:text-4xl font-bold container">
        {{ $t('home.aboutHeading') }}
      </h2>
      <div class="container grid grid-cols-2">
        <p v-html="$t('home.aboutBody1')"></p>
        <p v-html="$t('home.aboutBody2')"></p>
      </div>
    </div>
    <div
      id="projects"
      ref="projects"
      class="bg-green-600 w-full text-white py-4 md:py-8 space-y-8 md:space-y-16"
    >
    ...
    <div class="w-full h-48 md:h-96 bg-image-dm bg-cover bg-center relative">
      <div
        class="absolute right-4 bottom-4 bg-green-300 bg-opacity-90 p-2 rounded font-semibold text-green-900 text-sm"
      >
        {{ $t('home.captionDm') }}
    ...
    <Footer>
      ...
      <Connection to="/contact" icon="comment-alt-lines" icon-class="bg-yellow-500">
...
export default {
  components: { Footer, Connection, ProjectsList },
  methods: {
    scrollTo(view) {
...
<style>
.bg-image-bib {
  background-image: url('/img/background-bib.jpg');
}
.bg-image-dm {
  background-image: url('/img/background-dm.jpg');
}
